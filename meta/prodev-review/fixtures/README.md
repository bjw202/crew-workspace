...
## 토큰은 지운 것이다 (2026-09-11)

저장소가 public 이 되어 살핀 결과, `chat/minidiscord.db` 에 **살아 있는 봇 토큰 다섯과 로그인 세션 서른**이 그대로 있었다.
서버가 127.0.0.1 에만 열려 실해는 없었으나 공개 저장소에 둘 값이 아니다. (기록 `../runs/2026-09-11-public-check.md`)

**지금 두 fixture DB 는 토큰을 지운 것이다** (`../scripts/tools/db-scrub.js`) —
`bots.token` 은 `fixture-token-<id>` 이고 `sessions` 는 0행이다. **대화 377건과 나머지 표는 그대로**라 시험 값은 안 바뀐다.
시험이 토큰 값을 읽지 않으므로 `npm test` · `npm run test:server` 도 그대로다.

**다시 만들 때도 지운 판으로 준다.** 살아 있는 서버 DB 를 그대로 복사하지 않는다. 이 절은 `prodev/test/fixtures/README.md` 와 같은 글이다 (그 파일은 여기서 복사해 간다).
