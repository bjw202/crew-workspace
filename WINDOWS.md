...
| 무엇 | 왜 |
|---|---|
| ~~봇이 방의 글에 스스로 깨어나는 것~~ — **닫혔다** | **2026-09-13 에 사람이 PowerShell 창에서 켜 확인했다** — 채널이 붙고 방에서 부르면 봇이 깨어난다. 도구로는 못 재는 자리다: 채널이 글을 세션에 밀어 넣으려면 **대화형 창(TTY)** 이 필요하고, 파이프로 띄우면 `--print` 로 떨어진다. 그때 **PowerShell 의 백틱 이어쓰기가 붙여넣기에서 깨지는 것**도 같이 나왔다 (`README.md` '켜는 순서' ③ 의 상자 — 한 줄로 두라는 까닭) |
